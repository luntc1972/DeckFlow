...
| 05-02-03 | 02 | 2 | BRKT-02 | T-05-04, T-05-05, T-05-13 | Dialect parity gated by BOTH the dialect-independent parameter-type test AND a REQUIRED `[PostgresFact]` metadata round-trip (non-null `Theorycrafted`, all six values asserted). Postgres is positive-proof-only; a skipped or zero-test run is a task FAILURE to report as a blocker, not a "NOT VERIFIED" note to record (corrected 2026-08-01, round-5 review HIGH-1) | unit + gated integration | `WSLENV="${WSLENV:+$WSLENV:}DECKFLOW_POSTGRES_TESTS/w" DECKFLOW_POSTGRES_TESTS=1 dotnet.exe test DeckFlow.Web.Tests/DeckFlow.Web.Tests.csproj --filter FullyQualifiedName~PostgresStorageTests --no-restore` | Yes | pending |
...
Every task additionally runs `git add -A && scripts/format-check-changed.sh staged` as its plan's final `<automated>` step; CI `format-gate` is the authoritative enforcer. ⚠ The `git add -A` is required (corrected 2026-08-01, round-5 review LOW-1): the script reads only `git diff --cached` and exits 0 with "no changed C# files" against an empty index, so without staging it passes while inspecting nothing.
...
⚠ **Corrected 2026-08-01 (round-5 review, HIGH-1). This note previously said the Postgres path was "provable in **no** currently available environment" — that is false as of 2026-08-01.** Docker Desktop plus `PostgresContainerFixture` (`PostgresContainerFixture.cs:36-86`) runs it locally; the gated suite was verified at **2231 passed / 0 failed / 1 skipped** with every `[PostgresFact]` executing. The `[PostgresFact]` metadata round-trip is therefore **REQUIRED**, not optional, and the "**NOT VERIFIED on Postgres**" carry-forward is withdrawn: a skipped or zero-test run is a task failure to report as a blocker, never a recordable outcome. Only output showing `[PostgresFact]` tests **passed** counts.

`ArchidektDeckMetadataParametersTests` remains required and runs everywhere, but it is **not a substitute** — it asserts what `ArchidektDeckMetadataParameters.From` produces, never that the repository routes through it. Only the Postgres round-trip can separate a raw `bool?` bind from `From`'s `int?`, because `BoolTypeHandler` binds SQLite as `1/0` and Postgres as a native boolean (`DapperTypeHandlers.cs:98-116`).

**Prerequisite:** Docker Desktop must be running. It does not start automatically and does not persist across reboots. WSL integration is NOT required — the tests run under Windows `dotnet.exe` and reach the engine over the named pipe.
