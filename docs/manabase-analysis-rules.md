...
- **Conditional-untapped override** (`checkLandUntapped`, bundled under `analysis.manabase.accuracy`, default **ON**) — `IsConditionallyUntapped`:
  ...
  - **Explicitly out of scope** (stay tapped): **fast lands** ("two or fewer other lands") and **slow lands** ("two or more other lands") — their clause names no basic type, so `ConditionalUntappedTypes` returns empty and they never flip. Low/negative value in Commander (fast lands are tapped most of a long game; slow lands only untap late). Regression-guarded.
