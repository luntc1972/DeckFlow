...
> Both resolved by the planner exactly per the recommendations below (92-02): Q1 → freshness is Stage-0 of `PullAndClassifyAsync`; Q2 → widened to the `PullClassifyResult(Entries, Freshness)` wrapper record surfaced as a persistent banner.

1. **[RESOLVED — freshness runs as Stage-0 of every Pull]** **Should the freshness check run every Pull, or be a separate on-demand action?**
...
2. **[RESOLVED — widened to `PullClassifyResult(Entries, Freshness)` wrapper, surfaced as a persistent banner]** **Exact return-type/wiring for surfacing the freshness result to the page (progress-log line only, vs. a dedicated field/banner).**
