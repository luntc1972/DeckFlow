...
| 91-XX-07 (TBD at planning) | TBD | 0 | SYNC-11 / seed-drift detection | D-07 | Prod row absent from `index-seed.json` is emitted as `seed-drift` (only when the seed is available) | unit | `dotnet.exe test DeckFlow.Core.Tests/DeckFlow.Core.Tests.csproj --no-restore --filter "FullyQualifiedName~ReconcileClassifier"` | ❌ W0 | ⬜ pending |
...
| 91-XX-28 (revision - Codex BLOCK) | 91-01 | 0 | SYNC-17 / single-read-API contract | T-91-03 | `SeedIndexFileReader.Read` returning `SeedIndexReadResult` is the ONLY public read surface - there is NO `ReadNaturalKeys`/bare-set overload, so no consumer can obtain seed membership without the SeedAvailable flag (compile-time contract) | unit | `dotnet.exe test DeckFlow.Core.Tests/DeckFlow.Core.Tests.csproj --no-restore --filter "FullyQualifiedName~SeedIndexFileReader"` | ❌ W0 | ⬜ pending |
| 91-XX-29 (revision - Codex BLOCK) | 91-04 | 0 | SYNC-11 / seed-drift availability gate | T-91-25 | Classifier takes the availability-aware `SeedIndexReadResult`; with `SeedAvailable=false` it emits ZERO seed-drift discrepancies regardless of prod rows (incl. many seed_managed=true rows absent from the empty key set), while published-orphan/file-orphan/body-hash-mismatch are unaffected | unit | `dotnet.exe test DeckFlow.Core.Tests/DeckFlow.Core.Tests.csproj --no-restore --filter "FullyQualifiedName~ReconcileClassifier"` | ❌ W0 | ⬜ pending |
| 91-XX-30 (revision - Codex BLOCK) | 91-06 | 0 | SYNC-11 / orchestrator carries SeedAvailable | T-91-26 | `RunDryRunAsync` returns `ReconcileDryRunResult(SeedAvailable, Discrepancies)` with SeedAvailable taken straight from `SeedIndexFileReader.Read` (not inferred from the discrepancy list); an unavailable/missing seed yields SeedAvailable=false + zero seed-drift + a "seed unavailable" report notice | unit | `dotnet.exe test DeckFlow.Studio.Tests/DeckFlow.Studio.Tests.csproj --no-restore --filter "FullyQualifiedName~ReconcileOrchestrator"` | ❌ W0 | ⬜ pending |
| 91-XX-31 (revision - Codex BLOCK) | 91-08 | 0 | SYNC-12 / Apply seed-availability refuse | T-91-27 | `ApplyRemovalsAsync` independently REFUSES (zero `SetVisibilityAsync` calls) when the fresh `ReconcileDryRunResult.SeedAvailable==false`, BEFORE any stale-check or hide, even when a non-empty/stale reviewed-removal set is supplied - the refuse is on the raw SeedAvailable flag, not the discrepancy set | unit | `dotnet.exe test DeckFlow.Studio.Tests/DeckFlow.Studio.Tests.csproj --no-restore --filter "FullyQualifiedName~ReconcileCoordinator"` | ❌ W0 | ⬜ pending |
...
- [ ] `DeckFlow.Core.Tests/Content/SeedIndexFileReaderTests.cs` — 3-outcome reader: two-entry file -> (SeedAvailable, 2 keys); valid empty file -> (SeedAvailable, empty); missing file -> (unavailable, empty); malformed JSON -> (unavailable, empty); `Read` is the SOLE read API (no bare-set overload) (Codex-HIGH / Codex-BLOCK)
...
- [ ] `DeckFlow.Core.Tests/Content/ContentKbReconcileClassifierTests.cs` — four discrepancy classes, deterministic IDs, body hash reuse, seed-owned hide eligibility, file-orphan artifact-path identity + no path-inference rescue (Codex-LOW), seed-drift availability gate: SeedAvailable=false -> zero seed-drift regardless of prod rows / other classes unaffected (Codex-BLOCK)
...
- [ ] `DeckFlow.Studio.Tests/Services/ContentKbReconcileOrchestratorTests.cs` — dry-run drives the classifier, single prod read, seed excluded from file-orphan enum, report sections; `RunDryRunAsync` returns `ReconcileDryRunResult` with SeedAvailable from `SeedIndexFileReader.Read`; unavailable seed -> SeedAvailable=false + zero seed-drift + "seed unavailable" report notice (Codex-BLOCK)
- [ ] `DeckFlow.Studio.Tests/ViewModels/ReconcileCoordinatorTests.cs` — dry-run available with flag OFF (surfaces SeedAvailable), two-step re-validated apply, stale (seed-drift) apply rejected, destructive apply flag gate, removal-scoped Apply (mixed-class dry-run still applies seed-drift removals) (Codex-MED), Apply refuses on SeedAvailable=false BEFORE stale-check with zero hides even with a stale reviewed-removal set (Codex-BLOCK)
...
|----------|-------------|------------|-------------------|
| Studio operator dry-run review | SYNC-11 | Requires an operator git checkout, readable report review, and local Studio state inspection | Run Studio against a fixture checkout, trigger Reconcile dry-run with `sync.reconcile` OFF, confirm all four classes appear in the report/store and no prod writes occur; then rename/remove `index-seed.json` and confirm the page shows a "seed unavailable - seed-drift/removal skipped" notice (not an empty "no drift") |
| Studio dry-run -> Apply UX | SYNC-12 | Human confirmation and stale-state review are operator workflow, not just service logic | Run dry-run (surfacing multiple classes), mutate seed/prod fixture before Apply, confirm stale Apply is refused; make the seed unreadable and confirm Apply refuses with zero hides; rerun dry-run with matching state, flip `sync.reconcile` ON in the web DB, Apply, and confirm only seed-owned seed-drift removals are soft-hidden while non-removal classes are shown but untouched |
