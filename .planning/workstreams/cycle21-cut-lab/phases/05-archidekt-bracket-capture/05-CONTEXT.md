...
- **D-09: The URL-import commander-extraction fix is ratified into Phase 5.** `AdminHarvestController.SubmitUrl` selects the commander with `string.Equals(entry.Category, "Commander", ...)` (`AdminHarvestController.cs:269`). That predicate is **unreachable for Archidekt payloads**: `IsBoardCategory` (`DeckFlow.Core/Integration/ArchidektApiDeckImporter.cs:150`, applied at `:79` of that same file) strips `Commander` out of `userCategories` before `Category` is built, and `ArchidektApiDeckImporter.DetermineBoard` records the commander as `Board = "commander"` (`ArchidektApiDeckImporter.cs:130`). Consequently every URL-imported `deck_queue` row persists `commander_name = NULL` **always**, today. Phase 5 changes the predicate to `entry.Board == "commander"` with ordinal-ignore-case comparison, matching the already-correct bulk path (`ArchidektDeckCacheSession.cs:185-188`). This is a decision, not a suggestion: plan 05-03 implements it and tests it.
...
### URL-Upsert Preservation Granularity

- **D-10: The URL upsert gates metadata preservation per *record*, not per *column*.** Ratified by the user on 2026-07-29 after Round 3 plan review.
  - **Rule.** On `MarkUrlDeckProcessedAsync`'s `ON CONFLICT ... DO UPDATE` branch: a **non-null** `ArchidektDeckMetadata` record writes **all six** metadata columns verbatim — including writing NULL over a previously-captured value that Archidekt no longer declares. A **null** record leaves all six columns untouched (the BRKT-03 anti-wipe guarantee, unchanged). The discriminator is `excluded.archidekt_metadata_captured_utc IS NULL`, which holds exactly when the caller passed `metadata: null`, because `CapturedUtc` is non-null inside the record by contract.
  - **Why.** Per-column `COALESCE(excluded.archidekt_x, deck_queue.archidekt_x)` cannot express this. A captured record with `EdhBracket = null` — the legitimate captured-absent state defined by D-03 — binds NULL, so `COALESCE` retains the stale bracket. Concretely: URL-import deck X while Archidekt declares bracket 3 (row: bracket = 3, captured = T1); the owner clears it; re-import correctly captures `EdhBracket = null`; the row becomes `captured = T2, bracket = 3`, asserting that Archidekt declared bracket 3 at T2. That is false, permanent, and undetectable after the fact, and under D-03 that cell is defined provenance. It also makes "captured, absent" unreachable on the URL path and makes the same deck diverge by write path, against D-05.
  - **Effect on D-05.** With D-10 the URL upsert and the bulk `UPDATE` produce identical column state for identical metadata, which is what D-05 asks for.
  - **Supersession.** D-10 **overrides the Round 1 LEAD arbitration recorded as R-M3** in `05-REVIEWS.md` for the non-null case. R-M3 chose per-column `COALESCE` to mirror the single-field `commander_name` idiom at `DeckQueueRepository.cs:413`; the side effect it did not weigh is that mirroring a one-field idiom across six fields makes "captured, absent" unreachable. R-M3's null-record conclusion (preserve prior values) survives unchanged and is still correct; only its non-null-record conclusion is replaced. R-M3's record is retained in `05-REVIEWS.md`, marked superseded rather than deleted.
