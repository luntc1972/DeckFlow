...
public sealed class ContentKbSeedLoader : IContentKbSeedLoader
{
    ...
    private static ContentSiteIndexRow BuildRow(ContentKbSeedEntry entry)
    {
        ...
        return new ContentSiteIndexRow
        {
            ...
            // Why: the seed is exported from GetApprovedRowsAsync, so each seed entry is already
            // approved; loading it as the default pending status strands git-published rows unserved.
