...
- **Health verdict** — a four-tier scale that grades the *mana base*, not the curve:
  ...
  - **Needs work** — a real, broad shortage: a color is short by several sources, two or more colors are short, or the deck is two-plus lands short *and* the simulation actually shows that shortage biting. A paper land deficit alone never reds the verdict — a ramp-saturated deck whose cards all cast fine in the sim stays out of the red.
