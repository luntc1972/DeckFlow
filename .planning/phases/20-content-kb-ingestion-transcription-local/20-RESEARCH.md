...
- `GetUploadsAsync(channelId)` — returns `IAsyncEnumerable<PlaylistVideo>`, most recent first; use `.Take(N).ToListAsync()` to limit to recent N videos
...
public interface ITranscriptSource
{
    ...
    /// <summary>
    /// Fetches transcript for the given video. Returns null when captions are absent
    /// and Whisper is skipped (cap exceeded or ffmpeg failure); caller persists status.
    /// </summary>
...
// Verify presence at harvest start:
var checkResult = await TryRunProcessAsync(ffmpegExe, "-version");
if (!checkResult.Success)
    throw new InvalidOperationException("ffmpeg not found. Install ffmpeg and ensure it is on PATH.");

// Chunk command — segment copy, no re-encode, 10-minute segments (safely < 24MB for 64kbps mp3)
// ffmpeg -i input.webm -f segment -segment_time 600 -c copy -reset_timestamps 1 chunk_%04d.webm
var args = $"-i \"{inputPath}\" -f segment -segment_time 600 -c copy -reset_timestamps 1 \"{outputPattern}\"";
```
...
- Recommended segment duration: 600 seconds (10 min). At 64kbps opus/webm (YoutubeExplode audio default), 10 min ≈ 48MB → too large. At 128kbps mp3, 10 min ≈ 9.6MB → safe. Need to measure actual stream bitrate or use a more conservative 5-minute (300s) segment. [ASSUMED — bitrate of YoutubeExplode audio stream varies by video; 5-min/300s is safer default]
...
- **Blocking `IAsyncEnumerable` channel uploads:** `GetUploadsAsync` streams all videos lazily. Always apply `.Take(N).ToListAsync()` before iterating — otherwise Phase 21 would eventually fetch the entire channel history. Phase 20 should default to a configurable `--limit` option.
...
**How to avoid:** Phase 20 verifies ffmpeg presence at the start of any `harvest` run that could trigger Whisper (`Process.Start("ffmpeg", "-version")` and check exit code). Log a clear warning: "ffmpeg not found — audio >24MB will be marked failed."
...
**Why it happens:** The method lazy-streams YouTube's uploads playlist in batches; without a `Take()` call it will drain the entire playlist across many HTTP calls.
**How to avoid:** Always chain `.Take(limit).ToListAsync()` before processing. Phase 20's `harvest --limit N` option controls this. Phase 21's orchestrator knows which videos are already in `content_videos` and skips them.
...
var estimatedDurationMinutes = (double)streamInfo.Size.Bytes / (128_000 / 8.0) / 60.0;
var projectedCostUsd = (decimal)(estimatedDurationMinutes * 0.006);
var monthKey = DateTime.UtcNow.ToString("yyyy-MM");

if (await _spendLedger.WouldExceedCapAsync(projectedCostUsd, monthKey, ct))
{
    await _videoStore.UpdateTranscriptStatusAsync(videoId, TranscriptStatus.SkippedOverCap, ct);
    logger.Information("Skipping {VideoId} — projected cost ${Cost:F4} would exceed cap.", videoId, projectedCostUsd);
    return null;
}
```
...
- `ffmpeg`: absent on current machine. D-05 explicitly handles this: mark failed + continue. Phase 20 must verify at harvest-start and emit a warning: "ffmpeg not found on PATH — audio files >24MB will be marked 'failed'."
