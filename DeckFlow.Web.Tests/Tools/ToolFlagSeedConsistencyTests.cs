...
public sealed class ToolFlagSeedConsistencyTests : IDisposable
{
    ...
    public async Task EnsureSchemaAsync_SeedsAllNewToolFlags_AndPreservesExistingOverrides()
    {
        ...
        // (BRACKET-05), tool.creator-style.enabled (CS-30), and tool.primer.stale-flag
        // (PRIMER-01, phase 78). All other tool flags default to enabled.
        ...
        Assert.All(expectedKeys, key =>
        {
            ...
            if (key == "tool.bracket.enabled" ||
                key == "tool.creator-style.enabled" ||
                key == "tool.primer.stale-flag")
