...
test('previews, discards, and keeps a what-if swap without mutating state until Keep', async ({ page }) => {
  ...
  expect(cardOutOptionsBefore).not.toContain('Plains');
  ...
  expect(await page.locator('[data-cut-lab-whatif-delta-body] tr').count()).toBeGreaterThan(0);
  await expect(getMainStateInput(page)).toHaveValue(stateBeforePreview);
