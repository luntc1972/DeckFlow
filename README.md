...
### What's new — Mana Base accuracy: mulligan-aware verdict, graded health & cast delay
...
- **Two-tier health verdict:** the old binary OK / needs-work was replaced by a graded **Healthy / Functional / Needs work** read, so a single demanding card no longer flips the whole base to "needs work". *Functional* surfaces the demanding cards by name (e.g. *Functional — 1 demanding card: Grand Abolisher (77%)*).
...
- **Mulligan-aware, graded verdict:** per-color source requirements are derived from the simulation (modeling Commander's free first mulligan), so tight double-pips are no longer flagged against an inflated count; the overall health reads as a graded **Healthy / Functional / Needs work** verdict, with *Functional* listing the demanding cards by name rather than failing the whole base on one sub-threshold spell.
