...
| 98-01 T2 | 98-01 | 1 | CS-23 | T-98-01, T-98-02 | Fail-closed legality (null->reject); basic-land singleton exemption; CardNormalizer-based singleton (punctuation/DFC duplicates caught, not bare lowercase); pip castability | unit (pure Core) | `dotnet test DeckFlow.Core.Tests --filter FullyQualifiedName~CardGroundingRules` | ❌ new | ⬜ pending |
...
| 98-02 T2 | 98-02 | 2 | CS-21, CS-24 | T-98-11..T-98-15 | Strict accept returns canonical name; NotFound vs Ambiguous split; NotLegal fail-closed; blank candidate->NotFound with no resolver call; exact-batch collection 429/5xx/null->UpstreamUnavailable fail-closed (no fuzzy cascade); UpstreamUnavailable not cached; resolution-only cache (no cross-deck contamination); distinct cache-key prefix | unit | `dotnet test DeckFlow.Web.Tests --filter FullyQualifiedName~CardGroundingGuard` | ❌ new | ⬜ pending |
| 98-03 T1 | 98-03 | 3 | CS-22 | T-98-20, T-98-21, T-98-22 | Corpus-only pool; frequency-ranked + capped; deck-independent raw pool cached per creator (per-request guard filter never cached); every candidate re-validated through guard; zero direct Scryfall | unit | `dotnet test DeckFlow.Web.Tests --filter FullyQualifiedName~CreatorWhitelistPool` | ❌ new | ⬜ pending |
| 98-03 T2 | 98-03 | 3 | CS-22 | T-98-20, T-98-23 | ICreatorDeckCacheStore registered in Web via provider-aware connection factory (fixes latent CreatorProfileDeckCrawler reg); builder DI-resolves under ValidateOnBuild | build + DI-resolution unit | `dotnet build DeckFlow.Web -warnaserror` + `dotnet test DeckFlow.Web.Tests --filter FullyQualifiedName~CreatorWhitelistPool` | ❌ new | ⬜ pending |
