...
- [x] **Phase 26: Category Cache Schema Normalization (fresh-start)** — Normalize repeated deck/card TEXT into integer-keyed dimensions + compact indexes; full DB reset + re-harvest into new schema (no online migration) (DBO-01) — *off critical path; sequence before Phase 27* (functionally closed 2026-05-25: SC1/SC3/SC4/SC5 met, SC2 amended to achieved scope per b1a5cc8)
...
**Success Criteria** (what must be TRUE):
  ...
  2. `IContentHarvestOrchestrator.RunAsync()` flow: enabled sources → list videos → for new videos persist `content_videos` → captions first (free) → Whisper fallback ONLY when opt-in (`--enable-whisper`) AND captions absent AND local cap check passes; when Whisper is off, captions-absent videos are marked `skipped_no_captions` (non-terminal, re-attemptable) → summarize → clip → tag → emit artifact file + slim-index row → mark video `transcript_status` (`captions` | `whisper` | `failed` | `skipped_over_cap` | `skipped_no_captions`); staged-pipeline persistence (each stage own row + status) so a resumed run never re-Whispers or re-distills completed videos. (Phase 20 shipped captions-only-default + opt-in Whisper + `skipped_no_captions`; this SC reconciled 2026-05-27.)
  ...
  4. Source add + enable/disable is operable from the harvester (CLI verb) and respected by the next run (`is_enabled` filter). (SC4 reconciled 2026-05-27: "edit" = add-new + disable-old — name/url edits are not a mutate-in-place verb, to avoid slug-rename artifact orphaning per RESEARCH Q6.)
