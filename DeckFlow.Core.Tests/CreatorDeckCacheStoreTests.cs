...
{
    public sealed class CreatorDeckCacheStoreTestsPostgres : IClassFixture<PostgresContainerFixture>
    {
        private static readonly DateTimeOffset CachedUtc = DateTimeOffset.Parse("2026-07-11T18:05:00Z");
        private readonly PostgresContainerFixture _fixture;

        public CreatorDeckCacheStoreTestsPostgres(PostgresContainerFixture fixture)
        {
            _fixture = fixture;
        }

        [PostgresFact]
        public async Task UpsertAsync_ThenGetByCreator_RoundTripsMultiDeckSet_OnPostgres()
        {
            var store = await CreateStoreAsync();
            var expected = CreateDeckSet($"postgres-round-trip-{Guid.NewGuid():N}");
            ...
            var actual = await store.GetByCreatorAsync(expected[0].CreatorSlug);
        ...
        [PostgresFact]
        public async Task GetByCreatorAsync_EntriesJson_RoundTripsFieldForField_OnPostgres()
        {
            var store = await CreateStoreAsync();
            var expected = CreateEntry(
                $"postgres-fidelity-{Guid.NewGuid():N}",
                "deck-1",
                "hash-a",
                901,
                "Current",
            ...
            var actual = Assert.Single(await store.GetByCreatorAsync(expected.CreatorSlug));
        ...
        [PostgresFact]
        public async Task GetContentHashAsync_ReturnsStoredHashForHitAndNullForMiss_OnPostgres()
        {
            var store = await CreateStoreAsync();
            var expected = CreateEntry(
                $"postgres-hash-{Guid.NewGuid():N}",
                "deck-42",
                "canonical-hash-42",
                303,
                "Secondary",
            ...
            var hit = await store.GetContentHashAsync(expected.CreatorSlug, expected.DeckId);
            var miss = await store.GetContentHashAsync(expected.CreatorSlug, "missing-deck");
        ...
        [PostgresFact]
        public async Task GetByCreatorAsync_ScopesRowsByCreatorSlug_OnPostgres()
        {
            var store = await CreateStoreAsync();
            var creatorSlug = $"postgres-creator-a-{Guid.NewGuid():N}";
            var creatorA = CreateEntry(
                creatorSlug,
                "deck-1",
                "hash-a",
                ...
                CreateEntries());
            var creatorB = CreateEntry(
                $"postgres-creator-b-{Guid.NewGuid():N}",
                "deck-2",
                "hash-b",
                777,
                "Budget",
            ...
            await store.EnsureSchemaAsync();
            await store.UpsertAsync(creatorA);
            await store.UpsertAsync(creatorB);

            var actual = await store.GetByCreatorAsync(creatorSlug);

            var entry = Assert.Single(actual);
            Assert.Equal(creatorSlug, entry.CreatorSlug);
            Assert.Equal("deck-1", entry.DeckId);
        }

        private async Task<CreatorDeckCacheStore> CreateStoreAsync()
        {
            ...
            var descriptor = new RelationalDatabaseConnection(
                ...
                connectionString);
            return new CreatorDeckCacheStore(descriptor);
        }
