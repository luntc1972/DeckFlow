...
### Artifact saving (temporarily disabled)
The "Save artifacts to disk" and "Import saved session" features are **disabled in production** pending a refactor that moves artifact storage from the shared server volume to local browser download/upload. Server-side enumeration of saved sessions across users was a privacy leak. Until the refactor lands, all ChatGPT prompts and responses live only in the active browser session — copy/paste them into ChatGPT and back as before, but nothing is persisted server-side.
...
### Artifact saving (temporarily disabled)
The "Save artifacts to disk" feature is **disabled in production** pending a refactor (see ChatGPT Analysis Artifact saving section above). Comparison prompts and responses live only in the active browser session.
...
### Artifact saving (temporarily disabled)
The "Save artifacts to disk" feature is **disabled in production** pending a refactor (see ChatGPT Analysis Artifact saving section above). cEDH meta-gap prompts and responses live only in the active browser session.
