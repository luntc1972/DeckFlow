...
| 4 | Decide-sim starvation / tight per-assert timeouts on sim-heavy renders | genuine flake (load) | **Mitigated + documented + one concrete CI fix.** The decide-heavy cut-lab specs (`decide`/`tuning`/`whatif`/`theme-readability`) run a CPU-heavy `/api/cut-lab/decide` loop. Two manifestations: (a) running the full set on many *local* workers starves them (30s `Import pool → "Lock your pool"` timeout) — CI already pins `workers: 1` + `retries: 1`, and the specs pass green in isolation; local guidance = bounded `--workers=2` or run `theme-readability` in its own pass. (b) A pre-existing tight **5s default timeout** in `cut-lab-whatif.spec.ts` `expectCutsMadeRow` (post-what-if-Keep sim render) flaked on the CI 2-core runner while the same spec used `30_000` elsewhere — **FIXED** by giving those two assertions the same `{ timeout: 30_000 }` (not a Phase-111 regression; the spec is untouched since Phase 104). |
