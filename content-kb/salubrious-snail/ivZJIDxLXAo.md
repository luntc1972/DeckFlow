---
source: "Salubrious Snail"
title: "Your Pet Cards Deserve a Better Home"
url: "https://www.youtube.com/watch?v=ivZJIDxLXAo"
video_id: "ivZJIDxLXAo"
tags:
  archetype: ["midrange","control"]
  bracket: ["Core"]
  card_category: ["ramp","draw","removal","protection","recursion"]
generated_utc: "2026-07-12T20:13:36Z"
---

## Summary

The video advises how to place "pet cards" (cards a player loves) in Commander/EDH decks. It sorts pet cards into three groups. (1) Bad or hyper-specific cards: build a dedicated deck around their aesthetic or effect, accepting it won't be strong. (2) Cards that are simply good/flexible (examples: Dawn Charm, Swan Song, Kambal Consul of Allocation, and Shigeki—run in two ramp-heavy green decks for uncounterable instant-speed recursion): drop them into any deck of their colors. (3) Moderately powerful, somewhat niche cards—the hard middle—have two options. Option A: build a whole deck around one or two of them (the host did this for Astral Slide, Maze's End, and Time Sieve). Option B: put several into one flexible deck with a generic game plan; examples include a midrange Radha deck wanting 6-7 drops and 4-cost ramp, Hans's Jensen Carthalion five-color beatdown, and a low-power proliferate deck (Skeleton Ship) holding Overwhelming Intellect, Mind Unbound, Parallax enchantments, Grimoire of the Dead, storage lands; scalable up to Atraxa. Stated rule: the more generic a deck's plan, the more niche its card choices can be. Ensure a solid core (lands, ramp, draw) plus interaction—protection and removal. The host favors budget deckbuilding to serve many pet cards

## Key Clips

- **[02:03]** On Shigeki: the host runs it in two ramp-heavy green decks, where its flexible, uncounterable, instant-speed recursion makes the card consistently useful.
- **[02:35]** For niche moderately-powerful pet cards, option A is to take one or two and build a specific deck for them, rather than keeping ill-fitting cards in a deck just because you like them.
- **[03:15]** Building a deck where a pet card fully belongs is presented as the greatest act of love for that card; the host did this for Astral Slide, Maze's End, and Time Sieve.
- **[04:24]** His Radha deck runs all sorts of fun cards because it just wants 6-7 drops with meaningful game impact and 4-cost cards that vaguely gesture at ramp; it's budget and mid-power, so it can accommodate weaker cards.
- **[04:59]** Rule of thumb: a more generic deck plan permits more niche and varied card choices.
- **[05:17]** A precon-power proliferate deck (Skeleton Ship) can pack niche cards like Overwhelming Intellect, Mind Unbound, Parallax enchantments, and Grimoire of the Dead and have them all feel appropriate; scale up with Atraxa.
- **[06:36]** Don't forget interaction: protection for vulnerable elements of the deck and removal so opponents don't run away with the game—too often forgotten in pursuit of a fun vision.
- **[07:45]** He's a proponent of budget deckbuilding: instead of $500 on one fancy deck, build a half-dozen lower-budget decks serving different gameplay styles and different cards he loves.

## Tags

**Archetypes/Strategy:** midrange, control
**Format/Bracket:** Core
**Card Categories:** ramp, draw, removal, protection, recursion
