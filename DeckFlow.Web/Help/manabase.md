...
- **Unsupported interactions** — an honest disclosure (shown only when relevant) of cards the analysis cannot fully model: **X / variable-cost** spells are skipped from the castability simulation, and **hybrid / Phyrexian** pips carry no hard color requirement (correct per Karsten — they are payable more than one way — but it means their color need is approximated). These cards are listed by name so a clean verdict never quietly hides them.
