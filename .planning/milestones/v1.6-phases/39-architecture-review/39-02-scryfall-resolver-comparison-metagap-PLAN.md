...
<task type="auto">
  ...
  <action>
Create `DeckFlow.Web/Services/Scryfall/ScryfallCardResolver.cs`. Per project convention (CLAUDE.md naming patterns — e.g. `CardLookupService.cs` co-locates `ICardLookupService` + `ScryfallCardLookupService` + result records), the `IScryfallCardResolver` interface, its `sealed` impl, and any small result records all live together in this one file `ScryfallCardResolver.cs`. Namespace `DeckFlow.Web.Services.Scryfall` (a new sub-namespace — `Services/Scryfall/` does not yet exist; create the folder). The interface exposes:
