...
**Shipped:** Cycle 8 — Hardening & Backlog Burn-down (2026-06-17, CalVer `2026.06.4`) — 4 phases (51-54), 11 plans, 8/8 requirements satisfied (FEAT-01 PASS-WITH-NOTES). First CalVer release; a debt-burn cycle with no net-new user features. Verified the shipped v1.7 Studio/publish pipeline end-to-end (non-prod operator-UAT smokes + a **live prod publish run** that preserved admin flags on 86 pre-existing rows), fixed F-51-PG-01 (Postgres `42883` TEXT-vs-`timestamptz` in `AddDeckIdsAsync`), burned down the Phase 39 architecture backlog (CategoryKnowledgeRepository god-file split, `Program.cs` DI extraction + `Services/` foldering, deck-stat classifiers → Core, `Feedback*` dialect-leak removal — verifier PASS 8/8 + SECURED 4/4), and resolved feature debt (SpellbookCombo ranking fields + Deck Primer priority-rank; Gemini artifacts within the ~30k paste ceiling, flag default-off). v1.7 merged to `main`; Render deploys from `main`. Closed as tech-debt (no milestone audit; every phase individually verified). Build 0 errors; Core 447/447, Web 633/644 (11 PG-skip). Prior milestone: **v1.7 Local Harvest & Publish Studio** (2026-06-17, archived).

**Next:** **Cycle 9** (not yet scoped). Candidates: Studio / content-pipeline expansion (more creator sources, distill quality, automate harvest→distill→review→publish), SEO/growth/ops (Search Console + Bing + backlinks, analytics + monitoring, Core Web Vitals, on-site SEO + structured data), and SEED-001 (KB add/remove + publish-tracking). Start with `/gsd-new-milestone`.
