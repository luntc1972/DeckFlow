...
- [ ] **DECISION (record your choice before flipping `sync.directpush-gitbody`):**
  - [ ] **Option A — Accept-by-design.** Keep rows visible through updates (no flicker/outage on every edit). Correct the Stage-4 UI copy so it no longer claims rows stay "hidden + awaiting-confirm" for this update-to-visible case — the copy should instead say the row remains visible and may briefly serve the prior body until the deploy completes.
  - [ ] **Option B — Hide-then-reconfirm on update.** Change the update path to hide the row until the new body is confirmed deployed, mirroring the insert (new-row) path. This is a code change — file it as a follow-up task before choosing this option.
  - [ ] Record which option was chosen and the date: ______________________
- [ ] **Code fix stays DEFERRED regardless of which option is chosen** — Option A only requires a copy fix (small); Option B requires new DirectPush coordinator logic (not built this phase). Do not assume either is already implemented.
