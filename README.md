...
- **Four-tier health scale:** the verdict reads on a graded **Excellent / Solid / Workable / Needs work** scale that measures the *mana base*, not the curve. A high-mana-value bomb that casts late because it is expensive (a curve problem the base can't fix) no longer drags the verdict down — only a genuine, fixable color or land shortage does. *Needs work* is reserved for a real, broad shortage (lands 2+ short, a color short by several sources, or two-plus colors short); a single contained color issue is *Workable*; minor notes are *Solid*; a clean base is *Excellent*. Demanding cards are still surfaced by name.
