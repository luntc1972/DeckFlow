...
### Creator measured-style extractor (substrate)
- DeckFlow now includes a creator measured-style extraction substrate in `DeckFlow.Web`: crawl a curated creator's Archidekt decks, strip curated/personal staples, resolve multi-bucket categories from the harvested category repository with a Scryfall Tagger tail fill, compute category/lift/combo/Karsten measured metrics, and persist the resulting `MeasuredMetric[]` in `creator_style_profile`. This phase is substrate only; it does not add a user-facing page yet.

### Creator-Style critique
- **Creator-Style Critique (`/creator-style`, flag `tool.creator-style.enabled`, seeded OFF):** pick a creator, submit a deck by URL or pasted list, and DeckFlow builds a ChatGPT-ready critique packet grounded in exemplar decks and weighted numeric targets. The page uses a native creator picker with evidence-depth labels (`N decks · M videos`), a single URL-or-paste deck input form, and a copy-ready result block. When no creator profiles are loaded yet, the flag-on page shows an info state instead of the form.
