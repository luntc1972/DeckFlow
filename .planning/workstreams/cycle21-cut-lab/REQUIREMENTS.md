...
- [x] **RFLR-06**: A commander below the statistical bar, or a role that did not clear it, produces byte-identical floor defaults to current shipped behavior.
- [x] **RFLR-07**: The new `max(bracket, commander)` resolution has unit coverage for the commander-hit path, the fallback path, and the role-not-in-scope path. (wording aligned 2026-07-28 with RFLR-05's D-04 amendment; scope unchanged)
...
- [ ] **TWIN-01**: A new `CutLabFindingKind` groups unlocked, non-commander, non-land pool cards by (role ∩ **exact mana value** ∩ primary card type) and raises a finding when a group holds 3 or more **distinct normalized card identities**. Duplicate pool entries naming the same card collapse to one identity before the threshold is applied and before evidence is emitted, so three separate entries of one card never raise a finding. (amended 2026-07-29 by Phase 4 D-14 from "mana-value bucket" to exact mana-value equality: the phase goal says "at the same cost", so mana value 1 must not group with mana value 0; exact equality is a degenerate width-1 bucket and also fires less often, which protects Success Criterion 5's reviewable-density bar. The `["0-1","2","3","4","5+"]` boundaries used by `CurveCongestion` are deliberately not reused. Non-land and the `lands`/`other` role exclusions added per Phase 4 D-17.)
...
- [ ] **TWIN-03**: Evidence within a twin group is ordered highest mana value first, and the twin findings themselves are emitted highest mana value first, matching the community heuristic that the costlier of two functionally equivalent cards is the cut. (amended 2026-07-29 by Phase 4 D-14: under exact mana-value grouping every member of a group shares one mana value, so the within-group sort is degenerate and the requirement's intent is carried by the cross-group emission order. Both orderings are implemented and both are tested.)
...
| RFLR-05 | Phase 3 (conditional) | Satisfied |
| RFLR-06 | Phase 3 (conditional) | Satisfied |
| RFLR-07 | Phase 3 (conditional) | Satisfied |
| RFLR-08 | Phase 3 (conditional) | Satisfied |
