...
- **D-02 (RESOLVED 2026-07-20 — Option A locked by user):** Research (104-RESEARCH.md §"D-02 Metric Family Enumeration") found the 103 engine computes exactly **3 turn-indexed metrics**, not 5: `CommanderByTurn` (cap 3), `EngineByTurn` (cap 2), and `RepresentativeLineByTurn` (cap 4 — already blends payoff+interaction+tutor+engine). There is no standalone ramp-by-turn or interaction-by-turn metric. **DECISION: expose exactly these 3 editable turn goals** — no new `CutLabMetricKind` values, no new sim math. Relabel the blended one honestly in the goals UI (e.g. "Engine / payoff / interaction plan by turn N"). Seed defaults straight from `CedhMulliganCalibration`: commander=`TurnCapExplosive` (3), engine=`TurnCapEngine` (2), plan=`RepresentativeLineTurnCap` (4). Casual mode returns `int.MaxValue` for the representative-line cap → surface an "uncapped" state or hide that goal in casual (planner UI-copy call). Option B (add literal `RampByTurn`/`InteractionByTurn` kinds) is an explicit deferred fast-follow, only if UAT asks for literal rows.
