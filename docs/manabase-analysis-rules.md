...
| `analysis.manabase.ritual-land-credit` | **OFF** | cEDH only. Subtract `min(3.0, 0.5 × net-positive rituals)` from the enabled hybrid cEDH land target before the final `[22,45]` clamp. Separate from `ritual-burst-mana`; off = byte-identical. Calibration on the current 3281-deck harness moved the cEDH under-target rate from `21.8%` to `11.1%` and lowered the mean target by `0.7` lands. |
