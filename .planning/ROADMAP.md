...
**Verification status (2026-05-24):** Code complete + Codex peer-reviewed (RED iter-1 → YELLOW iter-2, both HIGH resolved → RED→GREEN). Build clean; Core 81/81; Web 463 pass / 13 pre-existing CSS fails / 5 PG-integration skipped. **Out-of-band gates pending user (away):** SC2 (≥50% index-footprint cut) and SC3 (EXPLAIN index scans) require a prod full-reset + re-harvest into the new schema (`docs/ops/db-full-reset.md`), then measure. Phase NOT formally closed until those run.
