...
| 260817-k2d | Isolate the Scryfall search cache keys. `ScryfallCardSearchService.SearchAsync` and `ScryfallCommanderSearchService.SearchAsync` both wrote to the shared `IMemoryCache` under an unprefixed lowercased query key with the same value type, so whichever ran first inside the 10-minute TTL silently poisoned the other — the commander typeahead could return non-commander cards. Three prefixes now, not two: `SearchCommandersAsync` keeps `commander:` because its query differs from the legendary-restricted one, which took `commander-legendary:`. Found as finding D-1 of the upstream cache-layer research | 2026-08-17 | a5a31320 | [260817-k2d-fix-scryfall-search-cache-key-collision](./quick/260817-k2d-fix-scryfall-search-cache-key-collision/) |
