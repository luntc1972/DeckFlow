...
/// <param name="ActualLands">Observed land count using the app's source classification.</param>
/// <param name="OldTarget">Historic five-argument cEDH land target.</param>
/// <param name="NewTarget">Enabled-context hybrid cEDH land target.</param>
/// <param name="HasBaseline">Whether the commander had a baseline sample of at least ten decks.</param>
...
public static class CedhCalibration
{
    ...
    public static CedhCalibrationReport Build(IEnumerable<CedhCalibrationRow> rows)
    {
        ...
        return new CedhCalibrationReport
        {
            ...
            CeilingHitCount = materialized.Count(row => Math.Abs(row.NewTarget - TargetCeiling) < 0.01),
            Segments = segments,
    ...
    public static string RenderMarkdown(CedhCalibrationReport report)
    {
        ...
        sb.AppendLine("| Segment | N | actual mean | old target | new target | under OLD% | under NEW% |");
        sb.AppendLine("|---------|---|------------|-----------|-----------|-----------|-----------|");
        foreach (CedhCalibrationSegmentStats segment in report.Segments)
        {
            sb.AppendLine(
                FormattableString.Invariant(
                    $"| {segment.Label} | {segment.SampleSize} | {segment.ActualLandsMean:0.0} | {segment.OldTargetMean:0.0} | {segment.NewTargetMean:0.0} | {segment.UnderOldPercent:0.0}% | {segment.UnderNewPercent:0.0}% |"));
        }

        sb.AppendLine();
        sb.AppendLine("## By commander (N≥10) — over-correction check (grindy Sisay/Tayam should stay ~healthy, not over-flagged-OK)");
        sb.AppendLine("| Commander | N | actual mean | old tgt | new tgt | under OLD% | under NEW% |");
        sb.AppendLine("|-----------|---|------------|--------|--------|-----------|-----------|");
        foreach (CedhCalibrationCommanderRollup commander in report.Commanders)
        {
            ...
            sb.AppendLine(
                FormattableString.Invariant(
                    $"| {EscapePipe(label)} | {commander.SampleSize} | {commander.ActualLandsMean:0.0} | {commander.OldTargetMean:0.0} | {commander.NewTargetMean:0.0} | {commander.UnderOldPercent:0} | {commander.UnderNewPercent:0} |"));
        }
    ...
    public static string RenderHeadline(CedhCalibrationReport report)
    {
        ...
        return FormattableString.Invariant(
            $"SampleSize={report.SampleSize}, UnderTarget={report.UnderOldPercent:0.0}% -> {report.UnderNewPercent:0.0}%");
    }
