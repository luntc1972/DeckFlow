...
""",
    ...
    [Fact]
    public async Task CacheKey_SameDeckDifferentPins_ProducesDifferentKey()
    {
        var service = CreateService();
...
""";
        ...
        var noPins = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"]
        }, default);
        var onePin = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
            PinnedVideoIds = ["vid-a"]
        }, default);
        var otherPin = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
            PinnedVideoIds = ["vid-b"]
        }, default);
        var normalizedPinsA = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
            PinnedVideoIds = ["vid-a", "vid-b"]
        }, default);
        var normalizedPinsB = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
            PinnedVideoIds = ["vid-b", "vid-a", "vid-a"]
        }, default);
        var lowerPin = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
        ...
        var upperPin = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
        ...
        var followA = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
        ...
        var followB = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
        ...
        var followC = await service.TryComputeCacheKeyAsync(new DeckAnalysisRequest
        {
            DeckSource = deckSource,
            TargetAiPlatform = "ChatGPT",
            SelectedAnalysisQuestions = ["strengths-weaknesses"],
            FollowedCreators = ["The Command Zone"]
        }, default);

        Assert.NotNull(noPins);
        Assert.NotNull(onePin);
        Assert.NotNull(otherPin);
        Assert.NotNull(normalizedPinsA);
        Assert.NotNull(normalizedPinsB);
        Assert.NotNull(lowerPin);
        Assert.NotNull(upperPin);
        Assert.NotNull(followA);
        Assert.NotNull(followB);
        Assert.NotNull(followC);
        Assert.NotEqual(noPins, onePin);
        Assert.NotEqual(onePin, otherPin);
        Assert.Equal(normalizedPinsA, normalizedPinsB);
        Assert.NotEqual(lowerPin, upperPin);
        Assert.Equal(followA, followB);
        Assert.NotEqual(followA, followC);
    }
