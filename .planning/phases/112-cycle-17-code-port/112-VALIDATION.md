...
- [ ] **Strengthen `CreatorStyleDiRegistrationTests.cs`** — port as-is, then add two things. (a) A companion assertion resolving `IArchidektOwnerClient` (and `CreatorProfileDeckCrawler`, which takes it directly) through the **real** `ArchidektOwnerClient`, not `FakeArchidektOwnerClient`: per D-20 / RESEARCH Finding 2 the faked version cannot detect the missing-`archidekt`-pipeline failure mode. (b) A whole-graph test resolving every service `AddDeckFlowCreatorStyle()` registers, derived from the `ServiceCollection` descriptor delta, with the five scoped services resolved inside an `IServiceScope`. Both are needed for D-13: (a) alone leaves thirteen registrations unproven, and the headless boot cannot cover them because Phase 112 wires no creator-style route. This is the most load-bearing test in the phase. *(Added 2026-07-24 after Codex plan review raised the gap.)*
