...
**Success Criteria** (what must be TRUE, assuming Phase 1 = go):
  ...
  4. The Cut Lab UI shows, per role floor with commander-specific data, BOTH the commander-derived value and the bracket+plan-derived value side by side, clearly labeled which is which — not just a single active value with a source tag. This is a genuine extension of the existing lands display, not a copy of it: `CutLabViewModel.cs:480-483` currently renders a commander-derived lands floor and a bracket-row lands floor identically as `Default for B5: N`, conveying no provenance at all — Phase 2 must fix that gap for lands too if it touches this rendering path, not just add dual-display for the five new roles. Roles/commanders without commander-specific data continue to show only the bracket+plan value.
**Plans**: TBD
