...
- **Unsupported-interaction disclosure:** cards the analysis can't fully model — **X / variable costs** (skipped from the castability simulation) and **hybrid / Phyrexian** pips (no hard color requirement, per Karsten) — are listed by name in a collapsible note so a clean verdict never silently hides them.
