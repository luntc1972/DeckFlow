...
- [x] **Phase 999.7: v1.3 Audit Cleanup — STATE Arithmetic + WDG Checkboxes + Stale Doc-Comments** — Land the documentation + state-tracking cleanup surfaced by `/gsd-audit-milestone v1.3` (audit at `.planning/v1.3-MILESTONE-AUDIT.md`): correct STATE.md progress arithmetic (`completed_phases:9 → 11`, `completed_plans:66 → 46`), flip 10 WDG-* REQUIREMENTS.md checkboxes from `[ ]` to `[x]`, update 5 stale Claude variant `<summary>` doc-comments (audit F-01; scope expanded from audit-listed 3 to repo-grep-confirmed 5 per Codex review HIGH #1 2026-05-22), document the intentional `/deck-analysis/download` short-circuit-tier omission (audit F-02), and finalize 999.5-UAT.md `status: testing → superseded-by-999.6`. No production-code behavior change. (planned 2026-05-22 on `v1.3` branch)
- [x] **Phase 999.8: Remove Legacy `chatgpt-*` 301 Redirects** — Strip the 11 permanent 301 redirects added by Phase 12 (Program.cs UseRewriter block, lines 357-371) that map legacy `chatgpt-packets/*`, `chatgpt-deck-comparison/*`, `chatgpt-cedh-meta-gap/*`, and `help/chatgpt-*` slugs to the AI-agnostic equivalents (`deck-analysis/*`, `deck-comparison/*`, `cedh-meta-gap/*`, `help/deck-analysis|deck-comparison`). User-decided 2026-05-22 that past links can be retired; redirects shipped 2026-05-08 (commit 5598f9d) as a graceful-deprecation tier and have been live for 2+ weeks — any external linkage that mattered would have been re-anchored by now. Scope: remove the `app.UseRewriter(...)` block and the `Microsoft.AspNetCore.Rewrite` using directive; verify no orphaned references in Razor views, JS, help markdown, or tests; build clean + suite green. No behavior change other than the 11 legacy URLs becoming 404 (intentional). (planned 2026-05-22 on `v1.3` branch — replaces the redirect-removal capture deferred from Phase 999.7 paused state)
...
**Success Criteria** (what must be TRUE):
  ...
  3. The 5 Claude variant files (`DeckFlow.Web/Services/PromptBuilders/Analysis/ClaudeAnalysisPromptVariant.cs`, `Comparison/ClaudeComparisonPromptVariant.cs`, `FollowUp/ClaudeFollowUpPromptVariant.cs`, `MetaGap/ClaudeMetaGapPromptVariant.cs`, `SetUpgrade/ClaudeSetUpgradePromptVariant.cs`) have their class-level `/// <summary>` doc-comments updated to drop "result-wrapped output" and replace with "direct JSON fenced-block output" (or equivalent matching the actual stripped behavior). Scope-expansion note (Codex review HIGH #1, resolved 2026-05-22 Option A): audit F-01 evidence inventory listed 3 files; independent `grep -rln 'result-wrapped' DeckFlow.Web/Services/PromptBuilders/` returns 5 — ClaudeFollowUp + ClaudeSetUpgrade also carry the stale phrase; SC3 scope is the full grep result.
...
- [x] 999.7-01-PLAN.md — Reconcile STATE.md progress counters against ROADMAP Progress table (SC1)
- [x] 999.7-02-PLAN.md — Flip REQUIREMENTS.md WDG-01..10 checkboxes to `[x]` (SC2)
- [x] 999.7-03-PLAN.md — Update 5 Claude variant class summaries (drop "result-wrapped output"); audit F-01 (SC3, SC6, SC7)
- [x] 999.7-04-PLAN.md — Add DeckController F-02 asymmetry comment + finalize 999.5-UAT.md status (SC4, SC5, SC6, SC7)

**Cross-cutting constraints:**

- `dotnet test DeckFlow.sln -c Release` reports Failed:0 Passed:497 Skipped:3 Total:500.

### Phase 999.8: Remove Legacy `chatgpt-*` 301 Redirects (INSERTED)

**Goal**: Strip the 11 permanent 301 redirects that Phase 12 (`feat(12-01)` commit `5598f9d` 2026-05-08; `fix(12) WR-02` `9593c25`) added to `DeckFlow.Web/Program.cs` (`UseRewriter` block, currently around lines 357-371) and remove the now-unused `Microsoft.AspNetCore.Rewrite` using directive. The 11 redirects map legacy `chatgpt-*` slugs to AI-agnostic equivalents:

| Legacy slug (regex)              | Target               |
|----------------------------------|----------------------|
...
| `^help/chatgpt-analysis/?$`      | `help/deck-analysis` |
| `^help/chatgpt-deck-comparison/?$` | `help/deck-comparison` |

User-decided 2026-05-22: past links can be retired. Redirects have been live for 2+ weeks; any external linkage that mattered would have been re-anchored by now. Removing reduces middleware overhead, simplifies the request pipeline, and closes the Phase 12 graceful-deprecation tier.

**Depends on**: Phase 999.7 (completes the v1.3 doc-cleanup before redirect strip; ordering keeps STATE.md arithmetic sane). Independent of harvester / worker code paths.

**Requirements**:
- D-01: Remove `app.UseRewriter(...)` block in `DeckFlow.Web/Program.cs` (currently lines ~357-371). The block is contiguous; full delete.
- D-02: Remove `using Microsoft.AspNetCore.Rewrite;` directive if no longer referenced anywhere in `Program.cs` (verify via grep; the using is currently at line 8 of Program.cs per Phase 12 diff).
- D-03: Verify no Razor view, JS bundle, help markdown, or test asserts the redirect behavior. If any found, surface them — do not silently delete.
- D-04: 404 on legacy URLs is intentional (no 410 Gone, no replacement page). Default ASP.NET Core 404 handler suffices.
- D-05: Single plan; mechanical edit; autonomous=true; Codex executes per project cross-AI rule.

**Success Criteria** (what must be TRUE):

  1. `DeckFlow.Web/Program.cs` no longer contains the `UseRewriter` block. `grep -c "UseRewriter\|AddRedirect" DeckFlow.Web/Program.cs` returns `0`.
  2. `DeckFlow.Web/Program.cs` no longer contains `using Microsoft.AspNetCore.Rewrite;` (or the directive remains only if another reference exists — surface either outcome).
  3. `grep -rn "chatgpt-packets\|chatgpt-deck-comparison\|chatgpt-cedh-meta-gap\|chatgpt-analysis" DeckFlow.Web/Views/ DeckFlow.Web/wwwroot/ DeckFlow.Web/Help/` returns 0 matches (no orphaned legacy-slug references that the redirects were silently catching).
  4. `"/mnt/c/Program Files/dotnet/dotnet.exe" build DeckFlow.sln -c Release` exits 0 with warning count <= post-999.7 baseline (Microsoft.AspNetCore.Rewrite unused-import warning if directive left in place would fail this gate).
  5. `"/mnt/c/Program Files/dotnet/dotnet.exe" test DeckFlow.sln -c Release` reports `Failed: 0`. Any test that asserted the redirect chain (e.g., 301 response with `Location: /deck-analysis`) must be updated to assert 404 OR removed if dead-premise.
  6. Manual smoke: `curl -I https://www.deckflow.gg/chatgpt-packets` returns `HTTP/2 404` (no longer `HTTP/2 301`). Verified post-deploy.

**Plans:** 1 plan

Plans:

- [x] 999.8-01-PLAN.md — Strip UseRewriter block + Microsoft.AspNetCore.Rewrite using; verify orphan references; SC1..SC6.

**Cross-cutting constraints:**

- `dotnet test DeckFlow.sln -c Release` reports Failed:0. Test baseline post-999.7 (likely still 497/3/500 since 999.7 is doc-only) preserved or updated if 999.7 closure changes the count.
...
| 999.7 v1.3 Audit Cleanup — STATE Arithmetic + WDG Checkboxes + Stale Doc-Comments | v1.3 | 4/4 | Complete    | 2026-05-22 |
| 999.8 Remove Legacy `chatgpt-*` 301 Redirects | v1.3 | 1/1 | Complete    | 2026-05-22 |
