(() => {
    ...
    const setTextIfChanged = (selector: string, value: string) => { const element = query<HTMLElement>(selector); if (element && element.textContent !== value) element.textContent = value; };
    const nextStep = () => {
        if (!draft) return 'Start by importing a baseline deck below.';
    ...
    const importDeck = async () => { const form = query<HTMLFormElement>('[data-deck-modules-import-form]'); const selectedSource = form?.querySelector<HTMLSelectElement>('select[name="DeckInputSource"]')?.value; const url = form?.querySelector<HTMLInputElement>('input[name="DeckUrl"]')?.value.trim() ?? ''; const pasteText = form?.querySelector<HTMLTextAreaElement>('textarea[name="DeckText"]')?.value.trim() ?? ''; const activeSource = selectedSource === 'PasteText' ? (pasteText ? 'PasteText' : url ? 'PublicUrl' : null) : (url ? 'PublicUrl' : pasteText ? 'PasteText' : null); if (!activeSource) { error('Enter a deck URL or paste a decklist.'); return; } try { const response = await request('/deck-modules/import', activeSource === 'PublicUrl' ? { activeSource, url } : { activeSource, pasteText }); const imported = await response.json() as Omit<Draft, 'version' | 'unassignedEntries' | 'coreEntries' | 'alternatives' | 'selectedAlternativeId'> & { importNotice?: string }; compilation = null; editingAlternative = false; draft = { ...imported, version, unassignedEntries: [...imported.baselineMainboardEntries], coreEntries: [], alternatives: [], selectedAlternativeId: '' }; const notice = query<HTMLElement>('[data-deck-modules-notice]'); if (notice) notice.textContent = imported.importNotice ?? ''; render(); announce('Baseline imported. Add two alternatives to begin assignment.'); } catch (exception) { error(exception instanceof Error ? exception.message : 'Deck import failed.'); } };
    ...
    const initialize = () => {
        ...
        page.addEventListener('submit', event => { if (event.defaultPrevented) return; if ((event.target as Element).matches('[data-deck-modules-import-form]')) { event.preventDefault(); void importDeck(); } }, { signal });
