...
**Shipped:** Cycle 12 — Manabase Accuracy, Command-Zone Awareness & Cross-Tool Persistence (2026-06-27, CalVer `2026.06.9`) — phases 70-74 + flag-key namespacing, shipped as trunk/ad-hoc work on `main` (not a single squash). Headlines: plain-language manabase verdict + metric glosses (Phase 71); manabase command-zone awareness + commander-castability lens (Phase 72); **deck-analysis command-zone awareness** — full command zone + optional companion designator in the `/deck-analysis` prompt artifact, awareness-only and byte-identical when off (Phase 73); cross-tool deck-input persistence with restore notice + Start-Over clear (Phase 74); Deck Primer output-style toggle (Moxfield-rich / Full cEDH); operator feature-flag key namespacing (`tool.*`/`service.*`/`analysis.*`/`manabase.*`) with state-preserving migration + Admin Flags prefix filter. Build 0/0; Web suite 929 pass; Phase 73 Codex-reviewed APPROVE + verifier PASS 6/6. New feature flags seeded OFF in prod. Prior milestones: **Cycle 11 — Security, Visibility Control & Creator-Lens** (2026-06-25, `2026.06.8`); **Cycle 10 — Studio Automation, Sync & Polish** (2026-06-21, `2026.06.6`, archived).

**Next:** Cycle 13 — deck-eval (in progress on branch `plan/cycle-13-deck-eval`). Carry-forward backlog still open: scheduled/bulk harvest (AUTO-03/04), SEO/growth lane (SEO-01..05). Operator follow-ups owed: flip the new Cycle 12 flags in prod when ready; manual prod deploy (autodeploy OFF).
